...
class Exhibitions extends CI_Controller {
	...
	{
		parent::__construct();
		$this->load->model(array('main_model', 'admin_model', 'exhibitions_model'));
	...
	public function index()
	{
		switch ($this->input->post('type')) {
			//刪除
			case 1:
				echo json_encode($this->exhibitions_model->deleteData($this->input->post('id')));
				break;
			default:
				$this->view();
				break;
	...
	private function view()
	{
		...
		$data['result'] = $this->exhibitions_model->getData($this->displayCount, $currentPaage);
		$data['menu'] = $this->main_model->getMenu();
		$data['page_title'] = '展覽列表';

		$this->load->view('backsite/header', $data);
		$this->load->view('backsite/navigation', $data);
		$this->load->view('backsite/exhibitions', $data);
	...
	{
		$config['base_url'] = base_url('/backsite/publications?g=1');
		$config['total_rows'] = $this->db->count_all_results('`gallery`.publications');
