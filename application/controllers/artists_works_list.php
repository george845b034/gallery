...
class Artists_works_list extends CI_Controller {
	...
	public function index()
	{
		...
		$this->totalRows = $this->artists_detail_model->getArtitsWithWork($id);
		$this->paginationStart();
		if(empty($id))redirect('artis');
		$data['artistsData'] = $this->artists_detail_model->getArtitsWithWorkForLimit($id, $this->displayCount, $currentPaage);
		$data['language'] = $this->about_model->checkLanguage();
		$data['headerData'] = $this->about_model->getHeaderData();

		if(count($data['artistsData']) <= 0)redirect('artis');

		$this->load->view('header', $data);
		$this->load->view('artists_works_list', $data);
	...
	{
		$config['base_url'] = base_url('artists_works_list?id=' . $this->id);
